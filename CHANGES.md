...
This log documents the changes to the original dataset. It is sorted in reverse chronological order (latest update to oldest update).
...
* Update starting time of MIDI and and Matchfiles to start with the audio files. This was done automatically by shifting the note onset/offset times and MIDI controls (pedals) using the ([`shift_files_to_audio.py`](./scripts/shift_files_to_audio.py)) script.

* Add script to setup the audio files ([`setup_audio.py`](./setup_audio.py)).
...
* **Unifying match file version**. In the original dataset, `Chopin_op10_no3` and `Chopin_op38` files are in the original match file version 1.0, while `Mozart_K331_1st-mov` and `Schubert_D783_no15` are version 5. Note however that the version 5.0 files in this dataset do not include pedal information that is available in other version 5.0 files (in the Magaloff and Zeilinger datasets; note that these datasets are not public). All files were converted to the same match file version.
